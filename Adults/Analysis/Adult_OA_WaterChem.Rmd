...
melted.filt$Date <- strptime(melted.filt$Date,format = "%m/%d/%y - %H:%M:%S")
melted$Date <- strptime(melted$Date,format = "%m/%d/%y - %H:%M:%S")
ggplot(melted.filt,aes(x=Date, y=value, colour = variable)) + geom_line() + xlab("Date") +ylab("Durafet pH") + ggtitle("pH values, filtered pH < 6.5")
```
```{r}
ggplot(melted,aes(x=Date, y=value, colour = variable)) + geom_line() + xlab("Date") +ylab("Durafet pH") + ggtitle("pH values, unfiltered")
```
...
ggplot(SW.chem,aes(x=Measure.Date, y=pH.Total, colour = Tank)) + geom_point() + xlab("Date") +ylab("Total pH (Benchtop)") + ggtitle("Total pH values")
```
...
```{r}
ggplot(SW.chem,aes(x=Measure.Date, y=Temperature, colour = Tank)) + geom_point() + xlab("Date") +ylab("Temperature (C)") + ggtitle("Temperature values")
```
```{r}
